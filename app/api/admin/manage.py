...
from app.services.grok.proxy_pool import ProxyPool
...
async def fetch_proxies(request: FetchProxiesRequest,
                       _: bool = Depends(verify_admin_session)) -> Dict[str, Any]:
    """
    从代理池获取代理列表
    ...
    try:
        ...
        logger.error(f"[Admin] 获取代理池失败: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail={"error": f"获取代理失败: {str(e)}", "code": "PROXY_FETCH_ERROR"}
        )


@router.post("/api/proxy/refresh")
async def refresh_proxies(request: FetchProxiesRequest,
                         _: bool = Depends(verify_admin_session)) -> Dict[str, Any]:
    """
    刷新代理池
    ...
    """
    try:
        logger.debug(f"[Admin] 刷新代理池 - 协议: {request.protocol}, 数量: {request.count}")
        ...
        logger.error(f"[Admin] 刷新代理池失败: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail={"error": f"刷新代理失败: {str(e)}", "code": "PROXY_REFRESH_ERROR"}
        )


@router.get("/api/proxy/list")
async def get_proxy_list(_: bool = Depends(verify_admin_session)) -> Dict[str, Any]:
    """
    获取当前代理池列表
    ...
    """
    try:
        logger.debug("[Admin] 获取代理池列表")
        
        proxies = ProxyPool.get_proxy_list()
        valid_count = sum(1 for p in proxies if p.get("valid") is True)
        ...
        logger.error(f"[Admin] 获取代理池列表失败: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail={"error": f"获取代理列表失败: {str(e)}", "code": "PROXY_LIST_ERROR"}
        )
